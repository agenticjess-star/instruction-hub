...
export async function aiCleanThread(raw: string): Promise<AiCleanResult> {
  try {
    // Calls the Vercel-hosted clean-thread function (migrated off Supabase edge functions).
    const base = import.meta.env.VITE_FUNCTIONS_URL ?? "";
    const res = await fetch(`${base}/api/clean-thread`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ raw }),
    });
    if (!res.ok) throw new Error(`clean-thread ${res.status}`);
